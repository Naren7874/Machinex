...
paymentSchema.methods.capturePayment = async function (): Promise<void> {
  const captureableStatuses = ['created', 'authorized'];
  if (!captureableStatuses.includes(this.status)) {
    throw new Error(`Payment cannot be captured. Current status: ${this.status}`);
  }
...
paymentSchema.methods.addRefund = async function (refundData: {
  ...
  if (!refundableStatuses.includes(this.status)) {
    throw new Error(`Payment cannot be refunded. Current status: ${this.status}`);
  }
  const currentTotalRefunded = this.refunds
    .filter((r: { status: string }) => r.status !== 'failed')
    .reduce((sum: number, r: { amount: number }) => sum + r.amount, 0);

  if (currentTotalRefunded + refundData.amount > this.amount) {
    throw new Error(
      `Refund amount ${refundData.amount} exceeds remaining balance ${this.amount - currentTotalRefunded}`
    );
  }
