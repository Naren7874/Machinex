...
conversationSchema.methods.markAsRead = async function (
  ...
  await mongoose
    .model('Conversation')
    .updateOne({ _id: this._id }, { $addToSet: { isReadBy: userId } });
  // Optionally reload if you need the updated document
  // await this.reload();
};
...
conversationSchema.methods.updateLastMessage = async function (message: {
  ...
  // Reset read status - only sender has "read" the new message
  this.isReadBy = [message.sender];
  await this.save();
};
